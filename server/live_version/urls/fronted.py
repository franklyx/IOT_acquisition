...
from handler.workgroup import WorkGroup, GetGroupByEquipment
...
fronted_handler = [
    ...
    ('/api/v1/realtime-data/(?P<factory_number>.*)', RealTimeData),
    ('/api/v1/alarm-data/(?P<parameter>.*)', AlarmInfo),
    ('/api/v1/history-data/(?P<parameter>.*)', HistoryInfo),
    ('/api/v1/report-data/(?P<parameter>.*)', ReportInfo),
    ('/api/v1/equipment-belong-group/(?P<parameter>.*)', GetGroupByEquipment)
