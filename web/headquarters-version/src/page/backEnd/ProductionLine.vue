<template>
    <div>
        <el-form :inline="true" class="demo-form-inline" ref="proline" @submit.native.prevent v-if="isFormStatus">
            <el-form-item label="车间名称">
                <el-select v-model="selectWorkShop" placeholder="请选择" @change="changeWorkShop">
                    <el-option v-for="item in workShopList" :key="item._id" :label="item.name" :value="item._id">
                    </el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="生产线名称">
                <el-input v-model="lineName" placeholder="生产线名称" @keydown.enter.native="addLine"></el-input>
            </el-form-item>
            <el-form-item>
    ...
    methods: {
        addLine() {
            addProductLine({
                ...
                name: this.lineName,
                workshop_id: this.selectWorkShop
            }).then(res => {
                if (res.code === 201) {
                    this.$message({
                        type: 'success',
                    ...
                    this.getProductLineFn()
                    this.$refs['proline'].resetFields()
                    console.log(this.$refs['proline'])
                }
            })
        },
